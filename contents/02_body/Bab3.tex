...
\begin{spacing}{1.5}
	...
	Penelitian ini mengkaji hubungan antara IOD, parameter oseanografi (arus, temperatur laut, salinitas, MLD, Chl-a, fluks air tawar, fluks panas bersih) dan meteorologi (laju presipitasi dan tekanan angin) di Samudera Hindia dengan koordinat ($0^\circ-24.6^\circ$ N) dan ($78.2^\circ-105^\circ$ E) (lihat Gambar \ref{fig:domain}).
...
%	\end{itemize}
	...
	\subsection[Analisis Korelasi]{Analisis Korelasi}
		...
		\begin{equation}
			...
			\end{aligned}
		\end{equation}
		Jika $\|t\|\geq t_{\text{tabel}}$ dengan ($\alpha=5\%$ atau $\alpha=1\%$) maka terdapat korelasi antara $x$ dan $y$.
\end{spacing}
\vspace{-0.5pc}
